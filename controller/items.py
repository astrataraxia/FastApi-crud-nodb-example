...
@router.get(path="",summary="2.0 모든아이템 조회 0921")
def read_items() -> Dict[str,dict]:
    return jsonable_encoder(temp_items)

@router.get("/{item_id}", summary="2.1 상품조회 0921") # GET /items/{item_id}
def read_item(item_id: ItemId) -> Item:
    item_or_404(item_id)
    return temp_items[item_id]

@router.post(path="",status_code=201, summary="2.2 상품등록 0921") # POST /items
...
@router.put("/{item_id}", status_code=202, summary="2.3 상품수정 0921")
...
@router.delete("/{item_id}", status_code=204, summary="2.4 상품수정 0921")
